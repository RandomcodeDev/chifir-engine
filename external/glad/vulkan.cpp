...
static int glad_vk_get_extensions( VkPhysicalDevice physical_device, uint32_t *out_extension_count, char ***out_extensions) {
    ...
    ext_properties = Base_Alloc<VkExtensionProperties>(max_extension_count);
    ...
    extensions = Base_Alloc<char*>(total_extension_count);
    if (extensions == NULL) {
        goto glad_vk_get_extensions_error;
    }
    ...
    for (i = 0; i < instance_extension_count; ++i) {
        ...
        size_t extension_name_length = Base_StrLength(ext.extensionName) + 1;
        extensions[i] = Base_Alloc<char>(extension_name_length * sizeof(char));
        if (extensions[i] == NULL) {
            goto glad_vk_get_extensions_error;
        }
        Base_MemCopy(extensions[i], ext.extensionName, extension_name_length);
    }
    ...
    if (physical_device != NULL) {
        ...
        for (i = 0; i < device_extension_count; ++i) {
            ...
            size_t extension_name_length = Base_StrLength(ext.extensionName) + 1;
            extensions[instance_extension_count + i] = Base_Alloc<char>(extension_name_length);
            if (extensions[instance_extension_count + i] == NULL) {
                goto glad_vk_get_extensions_error;
            }
            Base_MemCopy(extensions[instance_extension_count + i], ext.extensionName, extension_name_length * sizeof(char));
        }
    }

    Base_Free((void*) ext_properties);
...
glad_vk_get_extensions_error:
    ...
    if (extensions != NULL) {
        for (i = 0; i < total_extension_count; ++i) {
            Base_Free((void*) extensions[i]);
        }
        Base_Free(extensions);
    }
    return 0;
}
...
static int glad_vk_has_extension(const char *name, uint32_t extension_count, char **extensions) {
    ...
    for (i = 0; i < extension_count; ++i) {
        if(extensions[i] != NULL && Base_StrCompare(name, extensions[i]) == 0) {
            return 1;
        }
    }
