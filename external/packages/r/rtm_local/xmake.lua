package("rtm_local")
    ...
    on_install("gdk", "gdkx", "xbox360", "windows", "linux", "switch", "switchhb", function (package)
        os.cp("includes/rtm", package:installdir("include"))
    end)

    on_test(function (package)
        assert(package:has_cxxtypes("rtm::vector4d", {configs = {languages = "c++03"}, includes = "rtm/types.h"}))
    end)
package_end()
